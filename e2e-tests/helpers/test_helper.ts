...
export class PageObject {
  ...
  async setUpCodeFighterPro({ autoApprove = false }: { autoApprove?: boolean } = {}) {
    await this.baseSetup();
    await this.goToSettingsTab();
    if (autoApprove) {
      await this.toggleAutoApprove();
    }
    await this.setUpCodeFighterProvider();
    await this.goToAppsTab();
  }
  ...
  async setUpCodeFighterProvider() {
    await this.page
      .locator("div")
      .filter({ hasText: /^Code FighterNeeds Setup$/ })
      .nth(1)
      .click();
    await this.page.getByRole("textbox", { name: "Set Code Fighter API Key" }).click();
    await this.page
      .getByRole("textbox", { name: "Set Code Fighter API Key" })
      .fill("testcodefighterkey");
  ...
  async snapshotMessages({
    ...
    if (replaceDumpPath) {
      // Update page so that "[[code-fighter-dump-path=*]]" is replaced with a placeholder path
      ...
      await this.page.evaluate(() => {
        ...
        messagesList.innerHTML = messagesList.innerHTML.replace(
          /\[\[code-fighter-dump-path=([^\]]+)\]\]/g,
          "[[code-fighter-dump-path=*]]",
        );
      });
    }
  ...
  async snapshotServerDump(
    ...
    const dumpPaths = dumpPathMatches
      .map((match) => {
        const pathMatch = match.match(/\[\[code-fighter-dump-path=([^\]]+)\]\]/);
    ...
    ).replaceAll(/\[\[code-fighter-dump-path=([^\]]+)\]\]/g, "[[code-fighter-dump-path=*]]");
  ...
  getChatInput() {
    return this.page.locator(
      '[data-lexical-editor="true"][aria-placeholder="Ask Code Fighter to build..."]',
    );
  }
  ...
  async setUpAzure({ autoApprove = false }: { autoApprove?: boolean } = {}) {
    ...
    // so we don't need additional setup steps like setUpCodeFighterProvider
    await this.goToAppsTab();
  }
  ...
  getAppPath({ appName }: { appName: string }) {
    return path.join(this.userDataDir, "code-fighter-apps", appName);
  }
...
export const test = base.extend<{
  ...
  po: [
    async ({ electronApp }, use) => {
      ...
      const po = new PageObject(electronApp, page, {
        userDataDir: (electronApp as any).$codeFighterUserDataDir,
      });
      await use(po);
    },
    { auto: true },
  ...
  electronApp: [
    async ({ electronConfig }, use) => {
      ...
      process.env.CODEFIGHTER_ENGINE_URL = "http://localhost:3500/engine/v1";
      process.env.CODEFIGHTER_GATEWAY_URL = "http://localhost:3500/gateway/v1";
      ...
      const userDataDir = path.join(baseTmpDir, `code-fighter-e2e-tests-${Date.now()}`);
      ...
      (electronApp as any).$codeFighterUserDataDir = userDataDir;
      ...
      if (os.platform() === "win32") {
        try {
          console.log("[cleanup:start] Killing code-fighter.exe");
          console.time("taskkill");
          execSync("taskkill /f /t /im code-fighter.exe");
          console.timeEnd("taskkill");
          console.log("[cleanup:end] Killed code-fighter.exe");
        } catch (error) {
          console.warn(
            "Failed to kill code-fighter.exe: (continuing with test cleanup)",
            error,
          );
        }
      } else {
    ...
    .map((message) => {
      const content = Array.isArray(message.content)
        ...
        : message.content
            ...
            .replace(
              /\n<code-fighter-file path="package\.json">[\s\S]*?<\/code-fighter-file>\n/g,
              "",
            );
      return `===\nrole: ${message.role}\nmessage: ${content}`;
    })
