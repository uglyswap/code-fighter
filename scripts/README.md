...
1. `code-fighter-{version}-1.x86_64.rpm` (Linux RPM)
2. `code-fighter-{version}-full.nupkg` (Windows NuGet package)
3. `code-fighter-{version}.Setup.exe` (Windows installer)
4. `code-fighter-darwin-arm64-{version}.zip` (macOS Apple Silicon)
5. `code-fighter-darwin-x64-{version}.zip` (macOS Intel)
6. `code_fighter_{version}_amd64.deb` (Linux DEB)
