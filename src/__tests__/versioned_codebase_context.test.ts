...
describe("parseFilesFromMessage", () => {
  describe("code-fighter-read tags", () => {
    it("should parse a single code-fighter-read tag", () => {
      const input = '<code-fighter-read path="src/components/Button.tsx"></code-fighter-read>';
      const result = parseFilesFromMessage(input);
      expect(result).toEqual(["src/components/Button.tsx"]);
    });

    it("should parse multiple code-fighter-read tags", () => {
      const input = `
        <code-fighter-read path="src/components/Button.tsx"></code-fighter-read>
        <code-fighter-read path="src/utils/helpers.ts"></code-fighter-read>
        <code-fighter-read path="src/styles/main.css"></code-fighter-read>
      `;
      const result = parseFilesFromMessage(input);
    ...
    it("should trim whitespace from file paths in code-fighter-read tags", () => {
      const input =
        '<code-fighter-read path="  src/components/Button.tsx  "></code-fighter-read>';
      const result = parseFilesFromMessage(input);
      expect(result).toEqual(["src/components/Button.tsx"]);
    ...
    it("should skip empty path attributes", () => {
      const input = `
        <code-fighter-read path="src/components/Button.tsx"></code-fighter-read>
        <code-fighter-read path=""></code-fighter-read>
        <code-fighter-read path="src/utils/helpers.ts"></code-fighter-read>
      `;
      const result = parseFilesFromMessage(input);
      expect(result).toEqual([
    ...
    it("should handle file paths with special characters", () => {
      const input =
        '<code-fighter-read path="src/components/@special/Button-v2.tsx"></code-fighter-read>';
      const result = parseFilesFromMessage(input);
      expect(result).toEqual(["src/components/@special/Button-v2.tsx"]);
    });
  });

  describe("code-fighter-code-search-result tags", () => {
    it("should parse a single file from code-fighter-code-search-result", () => {
      const input = `<code-fighter-code-search-result>
src/components/Button.tsx
    ...
    it("should parse multiple files from code-fighter-code-search-result", () => {
      const input = `<code-fighter-code-search-result>
src/components/Button.tsx
...
src/utils/helpers.ts
    ...
    it("should skip empty lines in code-fighter-code-search-result", () => {
      const input = `<code-fighter-code-search-result>
src/components/Button.tsx
...
src/utils/helpers.ts
    ...
    it("should handle multiple code-fighter-code-search-result tags", () => {
      const input = `<code-fighter-code-search-result>
src/components/Button.tsx
src/components/Input.tsx
</code-fighter-code-search-result>
...
src/styles/main.css
  ...
  describe("mixed tags", () => {
    it("should parse both code-fighter-read and code-fighter-code-search-result tags", () => {
      const input = `
<code-fighter-read path="src/config/app.ts"></code-fighter-read>
...
<code-fighter-read path="src/components/Header.tsx"></code-fighter-read>
...
<code-fighter-read path="src/config/site.ts"></code-fighter-read>
...
`;
  ...
  describe("edge cases", () => {
    ...
    it("should handle malformed tags gracefully", () => {
      const input = `
<code-fighter-read path="src/file1.ts"
<code-fighter-code-search-result>
src/file2.ts
`;
      const result = parseFilesFromMessage(input);
    ...
    it("should handle nested angle brackets in file paths", () => {
      const input =
        '<code-fighter-read path="src/components/Generic<T>.tsx"></code-fighter-read>';
      const result = parseFilesFromMessage(input);
...
SRC/COMPONENTS/BUTTON.TSX
    ...
    it("should handle very long file paths", () => {
      ...
      const input = `<code-fighter-read path="${longPath}"></code-fighter-read>`;
      const result = parseFilesFromMessage(input);
...
describe("processChatMessagesWithVersionedFiles", () => {
  ...
  describe("processing assistant messages", () => {
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content:
            'I found this file: <code-fighter-read path="src/old.ts"></code-fighter-read>',
          providerOptions: {
            "code-fighter-engine": {
      ...
      const chatMessages: ModelMessage[] = [
        {
          ...
          content: [
            {
              type: "text",
              text: 'Here is the file: <code-fighter-read path="src/array.ts"></code-fighter-read>',
            },
            {
              type: "text",
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "user",
          content:
            'Check this: <code-fighter-read path="src/user-file.ts"></code-fighter-read>',
        },
      ];
      const appPath = "/test/app";
    ...
    it("should skip assistant messages without sourceCommitHash", async () => {
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content: 'File here: <code-fighter-read path="src/no-commit.ts"></code-fighter-read>',
          // No providerOptions
        },
        {
          role: "assistant",
          content:
            'Another file: <code-fighter-read path="src/no-commit2.ts"></code-fighter-read>',
          providerOptions: {
            // code-fighter-engine not set
          },
        },
      ];
      const appPath = "/test/app";
  ...
  describe("parsing multiple file paths", () => {
    it("should process multiple files from code-fighter-code-search-result", async () => {
      const { getFileAtCommit } = await import("@/ipc/utils/git_utils");
      const mockGetFileAtCommit = vi.mocked(getFileAtCommit);
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content: `<code-fighter-code-search-result>
src/file1.ts
src/file2.ts
</code-fighter-code-search-result>`,
          providerOptions: {
    ...
    it("should process mixed code-fighter-read and code-fighter-code-search-result tags", async () => {
      const { getFileAtCommit } = await import("@/ipc/utils/git_utils");
      const mockGetFileAtCommit = vi.mocked(getFileAtCommit);
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content: `
<code-fighter-read path="src/file1.ts"></code-fighter-read>
  ...
  describe("error handling", () => {
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content:
            'Missing file: <code-fighter-read path="src/missing.ts"></code-fighter-read>',
          providerOptions: {
            "code-fighter-engine": {
              sourceCommitHash: "commit1",
            },
    ...
    it("should handle getFileAtCommit throwing an error", async () => {
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content: 'Error file: <code-fighter-read path="src/error.ts"></code-fighter-read>',
          providerOptions: {
            "code-fighter-engine": {
              sourceCommitHash: "commit1",
            },
  ...
  describe("multiple messages", () => {
    it("should process multiple messages with different commits", async () => {
      ...
      const chatMessages: ModelMessage[] = [
        ...
        {
          role: "assistant",
          content: 'Here it is: <code-fighter-read path="src/file1.ts"></code-fighter-read>',
          providerOptions: {
            "code-fighter-engine": {
              sourceCommitHash: "commit1",
            },
        ...
        },
        {
          role: "assistant",
          content:
            'Here it is again: <code-fighter-read path="src/file1.ts"></code-fighter-read>',
          providerOptions: {
            "code-fighter-engine": {
  ...
  describe("integration with files parameter", () => {
    it("should combine files parameter with versioned files from messages", async () => {
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content: 'Old version: <code-fighter-read path="src/old.ts"></code-fighter-read>',
          providerOptions: {
            "code-fighter-engine": {
              sourceCommitHash: "abc123",
            },
