...
export function HelpDialog({ isOpen, onClose }: HelpDialogProps) {
  ...
  const isCodeFighterProUser = settings?.providerSettings?.["auto"]?.apiKey?.value;
...
`;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Need help with Code Fighter?</DialogTitle>
        </DialogHeader>
        <DialogDescription className="">
          If you need help or want to report an issue, here are some options:
        </DialogDescription>
        <div className="flex flex-col space-y-4 w-full">
          {isCodeFighterProUser ? (
            <div className="flex flex-col space-y-2">
              ...
              >
                <SparklesIcon className="mr-2 h-5 w-5" /> Chat with Code Fighter help
                bot (Pro)
              </Button>
              <p className="text-sm text-muted-foreground px-2">
                Opens an in-app help chat assistant that searches through Code Fighter's
                docs.
              </p>
            </div>
          ) : (
