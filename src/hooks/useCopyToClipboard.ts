...
export const useCopyToClipboard = () => {
  ...
  const copyMessageContent = async (messageContent: string) => {
    try {
      // Use the same parsing logic as CodeFighterMarkdownParser but convert to clean text
      const formattedContent = convertCodeFighterContentToMarkdown(messageContent);
  ...
  // Convert Code Fighter content to clean markdown using the same parsing logic as CodeFighterMarkdownParser
  const convertCodeFighterContentToMarkdown = (content: string): string => {
    if (!content) return "";

    // Use the same parsing functions from CodeFighterMarkdownParser
  ...
  // Convert individual custom tags to markdown (reuse the same logic from CodeFighterMarkdownParser)
  ...
  // Reuse the same parsing functions from CodeFighterMarkdownParser but simplified
