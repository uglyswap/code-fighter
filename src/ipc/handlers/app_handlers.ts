...
function listenToProcess({
  ...
  spawnedProcess.stdout?.on("data", async (data) => {
    ...
    // isNeon because: 1) only Neon apps (for the official Code Fighter templates) should
      ...
      if (urlMatch) {
        proxyWorker = await startProxy(urlMatch[1], {
          onStarted: (proxyUrl) => {
            safeSend(event.sender, "app:output", {
              type: "stdout",
              message: `[code-fighter-proxy-server]started=[${proxyUrl}] original=[${urlMatch[1]}]`,
              appId,
            });
          },
        });
...
async function executeAppInDocker({
  ...
  const containerName = `code-fighter-app-${appId}`;
  ...
  const dockerfilePath = path.join(appPath, "Dockerfile.code-fighter");
...
`;
  ...
  const buildProcess = spawn(
    "docker",
    ["build", "-f", "Dockerfile.code-fighter", "-t", `code-fighter-app-${appId}`, "."],
    {
      cwd: appPath,
      stdio: "pipe",
    },
  ...
  const process = spawn(
    ...
    [
      ...
      `${appPath}:/app`,
      "-v",
      `code-fighter-pnpm-${appId}:/app/.pnpm-store`,
      "-e",
      "PNPM_STORE_PATH=/app/.pnpm-store",
      "-w",
      "/app",
      `code-fighter-app-${appId}`,
      "sh",
      "-c",
...
export function registerAppHandlers() {
  handle("restart-code-fighter", async () => {
    app.relaunch();
    app.quit();
  });
      ...
      return withLock(appId, async () => {
        try {
          ...
          if (removeNodeModules) {
            ...
            if (runtimeMode === "docker") {
              logger.log(
                `Docker mode detected for app ${appId}. Removing Docker volumes code-fighter-pnpm-${appId}...`,
              );
              try {
                await removeDockerVolumesForApp(appId);
                logger.log(
                  `Removed Docker volumes for app ${appId} (code-fighter-pnpm-${appId}).`,
                );
              } catch (e) {
  ...
  ipcMain.handle("reset-all", async (): Promise<void> => {
    ...
    if (fs.existsSync(codeFighterAppPath)) {
      ...
      await fsPromises.mkdir(codeFighterAppPath, { recursive: true });
    }
