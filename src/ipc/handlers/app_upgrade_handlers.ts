...
const availableUpgrades: Omit<AppUpgrade, "isNeeded">[] = [
  ...
  },
  {
    ...
    manualUpgradeUrl: "https://codefighter.dev/docs/guides/mobile-app#upgrade-your-app",
  },
];
...
async function applyComponentTagger(appPath: string) {
  ...
  if (content.includes("plugins: [")) {
    if (!content.includes("codeFighterComponentTagger()")) {
      content = content.replace(
        "plugins: [",
        "plugins: [codeFighterComponentTagger(), ",
      );
    }
  } else {
    throw new Error(
