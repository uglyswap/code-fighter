...
import {
  ...
  getCodeFighterChatSummaryTag,
  getCodeFighterCommandTags,
...
async function getCodebaseTokenCount(
  ...
  const codebase = (
    await extractCodebase({
      appPath: getCodeFighterAppPath(appPath),
  ...
  return withLock("get-proposal:" + chatId, async () => {
    ...
    try {
      ...
      if (
        ...
        const proposalTitle = getCodeFighterChatSummaryTag(messageContent);

        const proposalWriteFiles = getCodeFighterWriteTags(messageContent);
        const proposalSearchReplaceFiles =
          getCodeFighterSearchReplaceTags(messageContent);
        const proposalRenameFiles = getCodeFighterRenameTags(messageContent);
        const proposalDeleteFiles = getCodeFighterDeleteTags(messageContent);
        const proposalExecuteSqlQueries = getCodeFighterExecuteSqlTags(messageContent);
        const packagesAdded = getCodeFighterAddDependencyTags(messageContent);
      ...
      if (latestAssistantMessage?.content) {
        const writeTags = getCodeFighterWriteTags(latestAssistantMessage.content);
        ...
        const commandTags = getCodeFighterCommandTags(latestAssistantMessage.content);
...
const approveProposalHandler = async (
  ...
  const chatSummary = getCodeFighterChatSummaryTag(messageToApprove.content);
