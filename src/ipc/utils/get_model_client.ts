...
export async function getModelClient(
  ...
  if (codeFighterApiKey && settings.enableCodeFighterPro) {
    ...
    if (providerConfig.gatewayPrefix != null || codeFighterEngineUrl) {
      ...
      logger.info(
        `\x1b[1;97;44m Using Code Fighter Pro API key for model: ${model.name} \x1b[0m`,
      );
      ...
      logger.warn(
        `Code Fighter Pro enabled, but provider ${model.provider} does not have a gateway prefix defined. Falling back to direct provider connection.`,
      );
