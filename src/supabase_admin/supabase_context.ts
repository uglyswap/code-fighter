...
async function getPublishableKey({ projectId }: { projectId: string }) {
  ...
  if (!publishableKey) {
    throw new Error(
      "No publishable key found for project. Make sure you are connected to the correct Supabase account and project. See https://codefighter.dev/docs/integrations/supabase#no-publishable-keys",
    );
  }
