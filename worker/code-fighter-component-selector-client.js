(() => {
  const OVERLAY_CLASS = "__code_fighter_overlay__";
  let overlays = [];
  ...
  function updateOverlay(el, isSelected = false) {
    ...
    const name = el.dataset.codeFighterName || "<unknown>";
    const file = (el.dataset.codeFighterId || "").split(":")[0];
  ...
  function removeOverlayById(componentId) {
    const index = overlays.findIndex(
      ({ el }) => el.dataset.codeFighterId === componentId,
    );
    if (index !== -1) {
  ...
  function onMouseMove(e) {
    let el = e.target;
    while (el && !el.dataset.codeFighterId) el = el.parentElement;
  ...
  function onClick(e) {
    ...
    if (selectedItem) {
      removeOverlayById(state.element.dataset.codeFighterId);
      window.parent.postMessage(
        {
          type: "Code Fighter-component-deselected",
          componentId: state.element.dataset.codeFighterId,
        },
        "*",
      );
      return;
    ...
    window.parent.postMessage(
      {
        ...
        component: {
          id: state.element.dataset.codeFighterId,
          name: state.element.dataset.codeFighterName,
        },
      },
      "*",
    );
  ...
  function onKeyDown(e) {
    ...
    if (key === "c" && hasShift && hasCtrlOrMeta) {
      ...
      window.parent.postMessage(
        {
          type: "Code Fighter-select-component-shortcut",
        },
        "*",
      );
    }
  ...
  window.addEventListener("message", (e) => {
    ...
    if (e.data.type === "activate-code-fighter-component-selector") activate();
    if (e.data.type === "deactivate-code-fighter-component-selector") deactivate();
    if (e.data.type === "clear-code-fighter-component-overlays") clearOverlays();
    if (e.data.type === "remove-code-fighter-component-overlay") {
  ...
  function initializeComponentSelector() {
    if (!document.body) {
      console.error(
        "Code Fighter component selector initialization failed: document.body not found.",
      );
      return;
    }
    setTimeout(() => {
      if (document.body.querySelector("[data-code-fighter-id]")) {
        window.parent.postMessage(
          {
            type: "Code Fighter-component-selector-initialized",
          },
          "*",
        );
        console.debug("Code Fighter component selector initialized");
      } else {
        console.warn(
          "Code Fighter component selector not initialized because no DOM elements were tagged",
        );
      }
    }, 0);
  }
